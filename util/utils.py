...
def detect(pred, coder, min_score, n_classes, max_overlap=0.45, top_k=200, is_demo=False):
    ...
    if coder.data_type == 'voc':
        n_classes = 20
    elif coder.data_type == 'coco':
        n_classes = 80
...
# FIXME detect for RetinaNet
def detect_retina(priors_cxcy, pred, min_score, max_overlap, top_k, n_classes=21):
    predicted_locs = pred[0]
