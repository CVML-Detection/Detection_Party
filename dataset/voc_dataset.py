...
from util.utils import bar_custom, voc_color_array
...
def download_voc(root_dir='D:\data\\voc', remove_compressed_file=True):
    ...
    print("VOC Dataset doesn't exist, Downloading Start...")
...
class VOC_Dataset(data.Dataset):
    ...
    def __init__(self, root="/data", split='train', download=True, resize=416):
        ...
        for i in os.listdir(root):
            if i.find('.tar') == -1 and i.find(self.split) != -1:
...
if __name__ == "__main__":
    ...
    root = window_root
    train_set = VOC_Dataset(root, split='test', download=True, resize=300)
