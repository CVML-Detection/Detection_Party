...
class VOC_Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = torch.LongTensor(labels)  # 0 ~ 19                             # FIXME
        img_name = torch.FloatTensor([img_name_to_ascii])
        additional_info = torch.FloatTensor([img_width, img_height])

        resize_size = 416
        image, boxes, labels = transform(image, boxes, labels, self.split, new_size=resize_size)
        ...
        if visualize:
            ...
            for i in range(len(boxes)):
                ...
                # print('----------------------------------------------------------------------------------')
                x1 = boxes[i][0] * resize_size
                y1 = boxes[i][1] * resize_size
                x2 = boxes[i][2] * resize_size
                y2 = boxes[i][3] * resize_size
