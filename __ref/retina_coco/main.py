...
def main():
    ...
    parser.add_argument('--epoch', type=int, default=30)                  # 14
    ...
    # 3. visdom
    vis = visdom.Visdom(port='8093')
    # 4. data set
    train_set = None
    test_set = None

    if opts.data_type == 'voc':
        train_set = VOC_Dataset(root=opts.data_root, split='train', resize=600)
        test_set = VOC_Dataset(root=opts.data_root, split='test', resize=600)
        opts.num_classes = 20

    elif opts.data_type == 'coco':
        train_set = COCO_Dataset(root=opts.data_root, set_name='train2017', split='train', resize=600)
        test_set = COCO_Dataset(root=opts.data_root, set_name='val2017', split='test', resize=600)
        opts.num_classes = 80
    ...
    # 6. network
    model = RetinaNet(base=Resnet_50(pretrained=True), num_classes=opts.num_classes).to(device)
    model = torch.nn.DataParallel(module=model, device_ids=device_ids)
    coder = RETINA_Coder(data_type=opts.data_type)  # 내부에 center_anchor 존재.
    ...
    # 9. scheduler
    scheduler = MultiStepLR(optimizer=optimizer, milestones=[10, 20], gamma=0.1)           # 9, 12
