...
def test(epoch, vis, test_loader, model, criterion, opts, coder):
...
if __name__ == "__main__":
    ...
    test_loader = torch.utils.data.DataLoader(test_set,
                                              ...
                                              num_workers=0)
    # 6. network
    model = SSD(VGG(pretrained=True), n_classes=test_opts.n_classes).to(device)
    model = torch.nn.DataParallel(module=model, device_ids=device_ids)
    ssd_coder = SSD_Coder(test_opts.data_type)

    # 7. loss
    criterion = MultiBoxLoss(ssd_coder)
